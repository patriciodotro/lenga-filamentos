...
function fmtG(n) { return String(n).replace(".",","); }
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .btn-sm { background: none; border: 1px solid #ff333344; color: #ff6666; border-radius: 6px; padding: 5px 10px; font-size: 11px; font-family: 'Montserrat', sans-serif; cursor: pointer; transition: all .2s; }
  .btn-sm:hover { background: #ff333311; }
  ...
  .loc-badge { display: inline-flex; align-items: center; gap: 4px; border-radius: 5px; padding: 2px 7px; font-size: 10px; font-weight: 600; letter-spacing: .04em; }
...
export default function App() {
  ...
  const handleRename = (lista, oldVal, newVal) => {
    const fieldMap = { materiales:"material", tipos:"tipo", marcas:"marca", colores:"color", estantes:"estante", posiciones:"posicion" };
    const field = fieldMap[lista];
    saveFil(filamentos.map(f => f[field]===oldVal ? {...f,[field]:newVal} : f));
    saveMov(movimientos.map(m => {
      const mf = field==="tipo" ? "tipo_fil" : field;
      return m[mf]===oldVal ? {...m,[mf]:newVal} : m;
    }));
  ...
  const handleImpresion = (lineas) => {
    ...
    toast_(`✓ Impresión registrada: ${lineas.length} filamento${lineas.length>1?"s":""}, ${fmtG(total)}g en total`);
  };
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0d0d0d",color:"#e0e0e0",fontFamily:"'Montserrat',sans-serif"}}>
      <style>{CSS}</style>
      {toast && <div style={{position:"fixed",top:16,right:16,background:"#0d1a00",border:"1px solid #4b7d0b55",borderRadius:10,padding:"12px 18px",fontSize:12,color:"#6fb010",zIndex:9999,letterSpacing:"0.02em",boxShadow:"0 8px 32px #00000077",maxWidth:340,fontWeight:600}}>{toast}</div>}
      ...
      <div style={{maxWidth:1000,margin:"0 auto",padding:"0 20px"}}>
        <div style={{borderBottom:"1px solid #1a1a1a",paddingTop:20}}>
          <div className="header-inner" style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8,paddingBottom:0}}>
            <img src={LOGO_SRC} alt="Lenga" style={{height:44,objectFit:"contain",filter:"brightness(0) invert(1)",marginBottom:4}}/>
  ...
  return (
    <div>
      ...
      <div className="stats-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:16}}>
        {[
          {label:"Stock total",      val:`${fmtG((totalStock/1000).toFixed(2))} kg`,sub:`${filamentos.length} tipos`,color:"#4b7d0b"},
      ...
      <div className="card">
        <div style={{fontSize:10,color:"#444",letterSpacing:".1em",textTransform:"uppercase",marginBottom:14,fontWeight:600}}>Detalle de stock</div>
        <div style={{display:"grid",gridTemplateColumns:"1.4fr 0.7fr 0.7fr 0.8fr 1fr 1.4fr 0.7fr",gap:10,padding:"0 0 10px",borderBottom:"1px solid #1e1e1e",marginBottom:2}}>
          {["Color","Material","Tipo","Marca","Stock","Ubicación","Valor"].map(h=>(
            <div key={h} style={{fontSize:9,color:"#333",letterSpacing:".08em",textTransform:"uppercase",fontWeight:600}}>{h}</div>
          ))}
        </div>
        {filamentos.length===0
          ?<div style={{color:"#333",fontSize:13,textAlign:"center",padding:"16px 0"}}>Cargá tu primer filamento en la pestaña Compra.</div>
          :filamentos.map(f=>{
            ...
            return (
              <div key={f.key} className="tbl-row" style={{display:"grid",gridTemplateColumns:"1.4fr 0.7fr 0.7fr 0.8fr 1fr 1.4fr 0.7fr",gap:10,alignItems:"center",borderBottom:"1px solid #1a1a1a",padding:"11px 0"}}>
                ...
                <div style={{fontSize:10,color:"#444",fontWeight:500}}>{f.marca}</div>
                <div>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:3}}>
                  {f.estante&&<span className="loc-badge" style={{background:`${ec}18`,border:`1px solid ${ec}40`,color:ec,fontWeight:600}}>{f.estante.replace("Estante ","")}</span>}
                  {f.posicion&&<span className="loc-badge" style={{background:"#1a1a1a",border:"1px solid #252525",color:"#666"}}>📍 {f.posicion}</span>}
                </div>
...
function FormImpresion({ filamentos, onSubmit }) {
  ...
  const submit=()=>{
    ...
    for(const l of validas){
      ...
      if(Number(l.gramos)>fil.stockGramos) return alert(`Stock insuficiente para ${fil.color} ${fil.material}. Disponible: ${fmtG(fil.stockGramos)}g`);
    }
              ...
              return (
                <div key={i} className="imp-row">
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                    <span style={{fontSize:11,color:"#666",fontWeight:600,letterSpacing:".06em"}}>FILAMENTO {i+1}</span>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                    ...
                    <div style={{display:"flex",flexDirection:"column",justifyContent:"flex-end"}}>
                      {fil&&<div style={{fontSize:11,color:"#555",padding:"11px 14px",background:"#0d0d0d",borderRadius:8,border:"1px solid #1e1e1e"}}>
                  ...
                  {fil&&linea.gramos&&restante!==null&&(
                    <div style={{marginTop:8,fontSize:11,color:"#555",padding:"8px 12px",background:"#0d0d0d",borderRadius:6,border:`1px solid ${restante<STOCK_MINIMO?"#cc444433":"#1e1e1e"}`}}>
...
function Historial({ movimientos }) {
  ...
  sorted.forEach(m=>{
    ...
    if(m.grupoId){
      ...
      rows.push({type:"impresion_grupo",data:grupo,fecha:m.fecha,id:m.grupoId});
    } else {
      rows.push({type:"impresion",data:m});
    }
  });
      ...
      <div className="card">
        {rows.length===0
          ...
          :rows.map((row,ri)=>{
            ...
            if(row.type==="impresion_grupo"){
              const grupo=row.data;
              const total=grupo.reduce((a,x)=>a+x.gramos,0);
              return (
                <div key={row.id} style={{padding:"12px 0",borderBottom:"1px solid #1a1a1a"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                    <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
                      <span style={{fontSize:9,padding:"3px 8px",borderRadius:4,letterSpacing:".06em",fontWeight:700,background:"#ffffff08",color:"#888",border:"1px solid #252525"}}>IMPRESIÓN</span>
                      <span style={{fontSize:11,color:"#555"}}>{grupo.length} filamento{grupo.length>1?"s":""}</span>
                    </div>
                    <div style={{textAlign:"right",flexShrink:0,marginLeft:12}}>
                      <div style={{fontSize:13,fontWeight:700,color:"#cc6666"}}>-{fmtG(total)}g</div>
                  ...
                  <div style={{marginTop:6,paddingLeft:8,borderLeft:"2px solid #252525",marginLeft:4}}>
                    {grupo.map(x=>(
                      <div key={x.id} style={{fontSize:11,color:"#555",padding:"2px 0"}}>
                        <span style={{color:"#aaa",fontWeight:500}}>{x.color} {x.material}</span>
                        <span style={{color:"#cc6666",marginLeft:8,fontWeight:600}}>-{fmtG(x.gramos)}g</span>
                        <span style={{color:"#333",marginLeft:8}}>{x.marca}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            return (
              <div key={m.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 0",borderBottom:"1px solid #1a1a1a"}}>
                <div style={{display:"flex",alignItems:"center",gap:12}}>
                  <span style={{fontSize:9,padding:"3px 8px",borderRadius:4,letterSpacing:".06em",fontWeight:700,background:"#ffffff08",color:"#888",border:"1px solid #252525"}}>IMPRESIÓN</span>
                  <div><span style={{fontSize:13,color:"#ccc",fontWeight:500}}>{m.color} {m.material}</span><span style={{fontSize:11,color:"#444",marginLeft:8}}>{m.tipo_fil}</span></div>
                </div>
                <div style={{textAlign:"right",flexShrink:0,marginLeft:12}}>
                  <div style={{fontSize:13,fontWeight:700,color:"#cc6666"}}>-{fmtG(m.gramos)}g</div>
                  <div style={{fontSize:10,color:"#333",marginTop:2}}>{new Date(m.fecha).toLocaleDateString("es-AR",{day:"2-digit",month:"short",year:"numeric"})}</div>
                </div>
              </div>
            );
...
function Maestros({ maestros, onAdd, onDelete, onRename }) {
  ...
  const saveEdit=newVal=>{
    ...
    if(maestros[editing.lista].includes(t)){alert("Ya existe ese valor.");return;}
